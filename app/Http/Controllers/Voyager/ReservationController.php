...
namespace App\Http\Controllers\Voyager;
    //***************************************
    //                File đã custome, không copy làm mẫu
    //             
    //               File đã custome, không copy làm mẫu
    //              
    //               File đã custome, không copy làm mẫu
...
class ReservationController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        if(Auth::user()->role_id == "1"){
            ...
            if (strlen($dataType->model_name) != 0) {
                // $model = app($dataType->model_name);
                //Chuyen Model query lan 1 lay stay_id theo host_id tu session
                $model = app('App\TStay');
                ...
                // Replace relationships' keys for labels and create READ links if a slug is provided.
                $dataTypeContent = $this->resolveRelations($dataTypeContent, $dataType);
                //Lấy stay_id theo user
                foreach($dataTypeContent as $value){
                    if(isset($value->stay_id))
                    {
                        $stay_id[] = $value->stay_id;
                    }                   
                }

                // Query lần 2 lấy tất cả record có chứa stay id ở trên trong bảng detail
                    $model = app('App\TReservationDetail');
                    ...
                    $query->whereIn($search->key, $search_value);
                    ...
                    // Replace relationships' keys for labels and create READ links if a slug is provided.
                    $dataTypeContent = $this->resolveRelations($dataTypeContent, $dataType);
                    //Lấy reservation theo id mới query
                        foreach($dataTypeContent as $value){
                            if(isset($value->reservation_id))
                            {
                                $reservation_id[] = $value->reservation_id;
                            }                   
                        }
                //Hoàn trả model query lấy comment
                $model = app($dataType->model_name);
                 
                if(isset($reservation_id)){
                    $search->key ="reservation_id";
                    $search_value = $reservation_id;
                }else{
                    $search->key ="reservation_id";
                    $search_value = [];
                }
