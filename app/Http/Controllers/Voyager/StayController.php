...
use App\TAddress;
use App\M_ward;
use App\TRoomGallery;
use App\M_province;
use App\M_district;

class StayController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        /* 
            Edit By Anh Khoa
        */
        // $roomGallery = TRoomGallery::where('stay_id', $id)->first();
    ...
    {        
        ...
        */
        ...
        $stay = TStay::where('stay_id', $id)->first();
        $user = User::where('id', Auth::id())->first();
        $address = TAddress::where('stay_id', $id)->first();
        $roomGallery = TRoomGallery::where('stay_id', $id)->first();

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted','stay','address','roomGallery','user'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        */
        ...
        $allUser = User::all();
        $address = TAddress::where('stay_id', $id)->first();
        $allAddress = TAddress::all();
        $allWard = M_ward::all();
        $allDistrict = M_district::all();
        // $stay = TStay::find($id);
        // $hostForStay = $stay->host()->get();

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','allUser','address','allAddress','allWard','allDistrict'));
    }
    ...
    {   
        ...
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        $dataType1 = Voyager::model('DataType')->where('slug', '=','t-stay')->first();
        ...
        if ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope'.ucfirst($dataType->scope))) {
            $model = $model->{$dataType->scope}();
            $model1 = $model1->{$dataType1->scope}();
        }
        if ($model && in_array(SoftDeletes::class, class_uses_recursive($model))) {
            $data = $model->withTrashed()->findOrFail($id);
            $data1 = $model1->withTrashed()->findOrFail($id);
        } else {
            $data = call_user_func([$dataType->model_name, 'findOrFail'], $id);
            $data1 = call_user_func([$dataType1->model_name, 'findOrFail'], $id);
        }
        ...
        // }else{
            ...
            // $this->insertUpdateData($request,'t-room-gallery', $dataType->editRows, $data1);
        // }
        ...
        */
        ...
        TStay::where('stay_id', $id)->delete();

        TStay::where('stay_id', $id)
        ->update([
        ...
        TAddress::where('stay_id', $id)->delete();

        TAddress::updateOrCreate([
            'stay_id'=> $id,
            'city_id'=> $request->city_id,
        ...
        if($request->file('img_url')!==null) {  
            ...
            TRoomGallery::where('stay_id', $id)->delete();

            TRoomGallery::updateOrCreate([
                'stay_id'=> $id,
                'img_url'=> $this->uploadImageMulti($slug,$request->file('img_url'))
            ]);
        }
    ...
    public function create(Request $request)
    {
        ...
        */
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','allUser','address','allAddress','allWard','allProvince'));
    }
    ...
    public function uploadImageMulti($slug,$img){
        $filesPath = [];
        $files = $img;
